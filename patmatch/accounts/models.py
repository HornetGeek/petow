...
class User(AbstractUser):
    ...
    phone = models.CharField(
        max_length=20, blank=True, default="", help_text="رقم الهاتف"
    )
    ...
    notify_breeding_requests = models.BooleanField(
        default=True, help_text="إرسال إشعارات طلبات التزاوج"
    )
    notify_adoption_pets = models.BooleanField(
        default=True, help_text="إرسال إشعارات الحيوانات المتاحة للتبني"
    )
...
class UserNotificationSettings(models.Model):
    ...
    min_minutes_between_non_transactional = models.PositiveSmallIntegerField(
        default=120
    )
...
class PhoneOTP(models.Model):
    ...
    def generate_otp(cls, user, phone_number):
        ...
        cls.objects.filter(user=user, phone_number=phone_number, is_used=False).update(
            is_used=True
        )
...
class PasswordResetOTP(models.Model):
    ...
    def generate_otp(cls, user):
        ...
        return cls.objects.create(
            user=user, email=user.email, otp_code=otp_code, expires_at=expires_at
        )
...
class AccountVerification(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.status == "approved":
            ...
            has_other_approved = (
                type(self)
                .objects.filter(user=self.user, status="approved")
                .exclude(pk=self.pk)
                .exists()
            )
    ...
    def send_approval_notifications(self):
        ...
        if not user:
            logger.warning(
                "AccountVerification %s has no user attached; skipping notifications",
                self.pk,
            )
            return
        ...
        body = (
            "تهانينا! تم اعتماد التحقق من حسابك ويمكنك الآن الاستفادة من جميع المزايا."
        )
...
class MobileAppConfig(models.Model):
    ...
    android_recommended_version = models.CharField(
        max_length=32, blank=True, default=""
    )
