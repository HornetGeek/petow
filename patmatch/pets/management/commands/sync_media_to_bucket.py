...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        s3_client, bucket = self._build_s3_client()
    ...
    def _build_s3_client(self):
        endpoint = getattr(settings, 'HETZNER_S3_ENDPOINT_URL', '').strip()
        region = getattr(settings, 'HETZNER_S3_REGION', '').strip()
        access_key = getattr(settings, 'HETZNER_S3_ACCESS_KEY', '').strip()
        secret_key = getattr(settings, 'HETZNER_S3_SECRET_KEY', '').strip()
        bucket = getattr(settings, 'HETZNER_MEDIA_BUCKET', '').strip()
        ...
        if missing:
            raise CommandError(f"Missing required Hetzner settings: {', '.join(missing)}")
